...
io.sockets.on('connection', function(socket){
	...
	socket.on('update_data', function(data){
		...
		let sql = `INSERT INTO device${data.device_id}_log(chieuquay, tocdo, Thoigian) values (  \'${data.chieuquay}\', \'${data.tocdo}\', \'${moment().tz('Asia/Ho_Chi_Minh').format('YYYY-MM-DD HH:mm:ss')}\')`;
		//console.log(sql);
	...
	socket.on('receive-motor', function(data){
		...
		con.query('SELECT unique_id FROM devices where device_id=?',[data.device_id], function(err,result, fields){
			...
			con.query(`UPDATE devices SET mode = ${data.mode} WHERE device_id = ${data.device_id}`, function(err){
				con.on('error', function(err){
					console.log('mysql error 148',err.code);
				});
			});			
